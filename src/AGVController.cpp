...
void AGVController::begin(const char* adminUser, const char* adminPass) {
    ...
    Serial.println("\n\n");
    Serial.println("╔════════════════════════════════════════╗");
    Serial.println("║   ESP32-S3 AGV Controller v2.0        ║");
    Serial.println("║   with WiFi Setup & Authentication    ║");
    Serial.println("╚════════════════════════════════════════╝");
    
    admin_username = String(adminUser);
    admin_password = String(adminPass);
...
void AGVController::startAPMode() {
    ...
    server->on("/savewifi", HTTP_POST, std::bind(&AGVController::handleSaveWiFi, this));
    server->onNotFound(std::bind(&AGVController::handleCaptivePortal, this));
